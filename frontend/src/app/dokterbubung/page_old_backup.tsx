...
type PrescriptionStatus = 'Pending' | 'Process' | 'Selesai' | 'Cancelled';
type PatientStatus = 'Waiting' | 'Examining' | 'Served';
...
const getNextLogId = (logs: Log[]): number => {
...
interface DoctorWorkstationProps {
    ...
    onUpdatePatientStatus: (patientId: string, status: PatientStatus) => void; // ✅ Added
}

const DoctorWorkstation: React.FC<DoctorWorkstationProps> = ({ medicines, onSubmitPrescription, patientQueue, onAddPatient, currentUser, onUpdatePatientStatus }) => {
    ...
    const addToCart = () => {
        ...
        // ✅ CHECK EXPIRY DATE
        if (isExpired(medToAdd.expiry)) {
            return alert(`❌ Obat ${medToAdd.name} sudah kadaluarsa (${medToAdd.expiry})!\nTidak dapat diresepkan.`);
        }

        if (isExpiringSoon(medToAdd.expiry)) {
            const confirmUse = confirm(`⚠️ Obat ${medToAdd.name} akan kadaluarsa pada ${medToAdd.expiry}.\nYakin ingin meresepkan?`);
            if (!confirmUse) return;
        }

        // ✅ CHECK STOCK AVAILABILITY
        const totalInCart = cart
            .filter(c => c.medicineId === medToAdd!.id)
            .reduce((sum, c) => sum + c.qty, 0);

        const availableStock = medToAdd.stock - totalInCart;

        if (qty > availableStock) {
            return alert(`❌ Stok tidak cukup!\n\nObat: ${medToAdd.name}\nStok tersedia: ${availableStock}\nDiminta: ${qty}`);
        }

        // ✅ CHECK FOR DUPLICATE (same medicine, same signa)
        const existingIndex = cart.findIndex(c => c.medicineId === medToAdd!.id && c.signa === signa);
        ...
        if (existingIndex >= 0) {
            ...
            setCart(updatedCart);
            alert(`✅ Jumlah ${medToAdd.name} ditambahkan menjadi ${updatedCart[existingIndex].qty}`);
        } else {
            // Add new item
            setCart([...cart, {
    ...
    const handleSubmit = () => {
        ...
        onSubmitPrescription({
            ...
            doctorName: currentUser.name, // ✅ Use currentUser instead of hardcoded
            items: cart,
            totalPrice: cart.reduce((acc, i) => acc + (i.price * i.qty), 0)
        });

        // ✅ Update patient status to served
        onUpdatePatientStatus(selectedPatient.id, 'Served');
                                            ...
                                            <div className="absolute top-full left-0 w-full bg-white shadow-xl border border-slate-200 rounded-lg mt-1 max-h-48 overflow-y-auto z-50">
                                                {filteredMedicines.length === 0 ? (
                                                    ...
                                                    filteredMedicines.map(m => {
                                                        ...
                                                        const expiringSoon = !expired && isExpiringSoon(m.expiry);

                                                        return (
                                                            <div
                                                                key={m.id}
                                                                className={`p-2.5 border-b border-slate-50 last:border-0 text-sm ${expired
                                                            ...
                                                            >
                                                                <div className="font-bold text-slate-700 flex items-center justify-between">
                                                                    <span>{m.name}</span>
                                                                    {expired && <span className="text-xs bg-red-600 text-white px-2 py-0.5 rounded font-bold">EXPIRED</span>}
                                                                    {expiringSoon && <span className="text-xs bg-orange-600 text-white px-2 py-0.5 rounded font-bold">⚠️ SEGERA KADALUARSA</span>}
                                                                </div>
                                                                <div className="text-xs text-slate-500 flex justify-between mt-1">
                                                                    <span>{m.location} • Exp: {m.expiry}</span>
...
const App = () => {
    ...
    const handleUpdatePatientStatus = (patientId: string, status: PatientStatus) => {
        setPatientQueue(patientQueue.map(p =>
    ...
    const handleProcessPrescription = (id: string) => {
        ...
        const insufficientStock = pres.items.some(item => {
            const med = medicines.find(m => m.id === item.medicineId);
            return !med || med.stock < item.qty;
        });

        if (insufficientStock) {
            alert('❌ Stok tidak cukup untuk memproses resep ini!');
            return;
        }

        // Logic pengurangan stok dengan OUT logging
        const updatedMeds = medicines.map(m => {
            ...
            return item ? { ...m, stock: Math.max(0, m.stock - item.qty) } : m; // ✅ Prevent negative
        });

        setMedicines(updatedMeds);

        // ✅ Update prescription with history log
            ...
            p.id === id ? {
                ...
                historyLogs: [
                    ...p.historyLogs,
                    {
                        status: 'Process',
                        time: getCurrentTime(),
                        note: `Diproses oleh ${currentUser.name}`
                    }
                ]
            } : p
        ));
            ...
            p.id === id ? {
                ...
                historyLogs: [
                    ...p.historyLogs,
                    {
                        status: 'Selesai',
                        time: getCurrentTime(),
                        note: `Diselesaikan oleh ${currentUser.name}`
                    }
                ]
            } : p
        ));
    ...
    const handleRestock = (id: string, amount: number, name: string) => {
        ...
        // ✅ Use utility function for log ID
        setLogs([...logs, {
            id: getNextLogId(logs),
            date: getCurrentDate(),
            type: 'IN',
